...
for file_path in tqdm.tqdm(train_directories):
    with open(file_path, encoding='utf8') as file:
        ...
        train_dataset_dict["input_text"] = train_dataset_dict.get("input_text") + [line.split('\t')[0] for line in lines if len(line.split('\t')) == 2]
        train_dataset_dict["target_text"] = train_dataset_dict.get("target_text") + [line.split('\t')[1] for line in lines if len(line.split('\t')) == 2]
        lines = None

train_df = pd.DataFrame(train_dataset_dict)
train_df = train_df.replace(to_replace='', value=np.nan).dropna()
train_ds = Dataset.from_pandas(train_df)
train_ds.to_csv("../prepared/train/par3-dipper-small/train_combined_sents_1.csv")
...
for file_path in tqdm.tqdm(valid_directories):
    with open(file_path, encoding='utf8') as file:
        lines = file.readlines()
        valid_dataset_dict["input_text"] = valid_dataset_dict.get("input_text") + [line.split('\t')[0] for line in lines if len(line.split('\t')) == 2]
        valid_dataset_dict["target_text"] = valid_dataset_dict.get("target_text") + [line.split('\t')[1] for line in lines if len(line.split('\t')) == 2]
        lines = None

valid_df = pd.DataFrame(valid_dataset_dict)
valid_df = valid_df.replace(to_replace='None', value=np.nan).dropna()
valid_ds = Dataset.from_pandas(valid_df)
valid_ds.to_csv("../prepared/validation/par3-dipper-small/validation_combined_sents_1.csv")