...
def calculate_similarity_and_get_path(
    ...
    df[created_para_sims_columns] = df.apply(lambda row:
                                             evaluate(sim_object, row['kgw-watermarked'], [f"pp-kgw-para-{i}" for i in range(1, no_paraphrases+1)]),
                                             axis='columns',
                                             result_type='expand')
    ...
    df[created_sent_sims_columns] = df.apply(lambda row:
                                             evaluate(sim_object, row['kgw-watermarked'], [f"pp-kgw-sent-{i}" for i in range(1, no_paraphrases + 1)]),
                                             axis='columns',
                                             result_type='expand')

    created_word_sims_columns = [f"pp-para-sim-{i}" for i in range(1, no_paraphrases + 1)]
    df[created_word_sims_columns] = df.apply(lambda row:
                                             evaluate(sim_object, row['kgw-watermarked'], [f"pp-kgw-word-{i}" for i in range(1, no_paraphrases + 1)]),
                                             axis='columns',
                                             result_type='expand')
...
if __name__ == "__main__":
    calculate_similarity_and_get_path(
        input_dir="../../processed/cleaned_paraphrased/mistralai-Mistral-7B-Instruct-v0_2_4_clean_paraphrased_01_03_2024.csv"
    )